    ...
    <div className={`min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-slate-100 px-4 py-10 ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className="mx-auto w-full max-w-md rounded-[32px] border border-slate-200 bg-white px-8 py-10 shadow-panel">
        <div className="grid gap-5 text-center">
          ...
          <div>
            <h1 className="text-3xl font-semibold text-slate-900">{t('brandTitle')}</h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">{t('brandSubtitle')}</p>
          </div>
        </div>

        <form className="mt-10 space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-slate-700">{t('emailOrMobile')}</label>
        ...
        <p className="mt-8 text-center text-sm leading-6 text-slate-500">{t('loginNote')}</p>

        <div className="mt-8 flex flex-wrap justify-center gap-4 text-center text-xs uppercase tracking-[0.24em] text-slate-400">
