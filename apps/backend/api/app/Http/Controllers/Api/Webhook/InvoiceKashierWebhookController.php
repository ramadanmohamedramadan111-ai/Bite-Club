<?php

namespace App\Http\Controllers\Api\Webhook;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use App\Services\Infrastructure\Payment\PaymentGatewayInterface;
use App\Services\Domain\Invoice\InvoiceDomainService;

class InvoiceKashierWebhookController extends Controller
{
    public function __construct(
        private readonly PaymentGatewayInterface $paymentGateway,
        private readonly InvoiceDomainService $invoiceDomainService
    ) {}

    public function handle(Request $request)
    {
        $payload = $request->all();
        
        Log::info('Kashier Invoice Webhook Payload:', $payload);

        $kashierSignature = $request->header('x-kashier-signature');
        
        // Platform keys
        $paymentApiKey = config('payment.kashier.api_key');

        if (!$paymentApiKey || !$this->paymentGateway->validateWebhookSignature($payload, $kashierSignature, $paymentApiKey)) {
            Log::error('Kashier Invoice Webhook: Invalid Signature or Missing API Key', ['signature' => $kashierSignature]);
            return response()->json(['message' => 'Invalid signature'], 400);
        }

        $event = $payload['event'] ?? null;
        $data = $payload['data'] ?? [];
        $orderId = $data['merchantOrderId'] ?? null;

        if ($event === 'pay') {
            $status = $data['status'] ?? null;
            $transactionId = $data['transactionId'] ?? null;

            if ($orderId) {
                $this->invoiceDomainService->handlePaymentWebhook((string)$orderId, (string)$status, $transactionId);
            }
        }

        return response()->json(['message' => 'success'], 200);
    }
}
